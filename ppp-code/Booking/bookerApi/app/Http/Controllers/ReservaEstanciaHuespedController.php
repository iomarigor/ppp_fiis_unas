...
class ReservaEstanciaHuespedController extends Controller
{
        ...
        if(!empty($datos)){
            ...
            if ($validar->count()==0){
                ...
                if ($validator->fails()) {
                    ...
                    $huesped->estado_registro=$datos["estado_registro"];
                    $huesped->save();
    ...
    public function registra_estancia_huesped_reserva(Request $request){
         $datos = array( 
			"numero_documento"=>$request->input("nDocumento"),
			"razon_social_nombre"=>$request->input("nombres")." ".$request->input("apellidos"),
			"nombre_comercial"=>$request->input("nombres")." ".$request->input("apellidos"),
			"nacionalidad"=>$request->input("nacionalidad"),
        ...
        $persona_id=-1;
        if($validar->count()==0){
            $persona_id=DB::table('persona')->insertGetId($datos);
        }else{
            $persona_id=$validar[0]->persona_id;
            $tDatos=array(
                "numero_documento"=>$request->input("nDocumento"),
			"razon_social_nombre"=>$request->input("nombres")." ".$request->input("apellidos"),
			"nombre_comercial"=>$request->input("nombres")." ".$request->input("apellidos"),
            ...
            Persona::where("persona_id", $persona_id)->update($tDatos);
        }
        $max_correlativo= DB::table('reserva_estancia_huesped')->max('correlativo');
        $numeracion = $max_correlativo+1;   
        $reserva_estancia=DB::table('reserva_estancia_habitacion')
                ->where('reserva_estancia_id',$request->input("reserva_estancia_id"))
                ->select('habitacion_id')
                ->first();
                if($request->input("reserva_estancia_huesped_id")!=null){
                    ReservaEstanciaHuesped::where("reserva_estancia_huesped_id", $request->input("reserva_estancia_huesped_id"))->update(array('estado_registro'=>1));
                }else{
                    $huesped = new ReservaEstanciaHuesped();
                    $huesped->persona_id =$persona_id;
                    $huesped->medio_transporte = null;
                    $huesped->procedencia_id = null;
                    $huesped->destino_id = null;
                    $huesped->reserva_estancia_id = $request->input("reserva_estancia_id");
                    $huesped->habitacion_id = $reserva_estancia->habitacion_id;
                    $huesped->correlativo = $numeracion; 
                    $huesped->motivo_viaje = null;
                    $huesped->descripcion_motivo_viaje = null;
                    $huesped->aereolinea = null;
                    $huesped->numero_vuelo = null;
                    $huesped->ubica_persona=0;
        ...
        $json = array(
                "status"=>200,
                "mensaje"=>"Estancia registrada",
                "detalles"=>null
            );
        } catch (\Throwable $th) {
            $json = array(
                "status"=>400,
                "mensaje"=>"Error al registradar estancia",
                "detalles"=>null
            );
        }
