...
use App\Mail\RegistroHuespedMail;
...
class ReservaEstanciaController extends Controller
{
    ...
    public function index(Request $request){
            ...
            ->join('users as em','re.empleado_id','=','em.id')
            ...
            ->select('re.reserva_estancia_id','re.estado_registro','re.fecha_reserva','re.fecha_confirmacion','re.fecha_checkin',
            ...
            'pt.descripcion','reh.early_checkin','reh.late_checkout','em.name as nombres','per.telefono','per.correo_electronico',
            ...
            're.tipo_especificacion','re.codigo_reserva','re.paquetes_turisticos_id','re.canales_venta_reserva_id'
            ,'per.razon_social_nombre','reh.habitacion_id','reh.tarifa',
            'pt.descripcion','re.paquetes_tarifa','reh.early_checkin','reh.late_checkout','pt.numero_noches','consumo.subtotal','em.name','per.telefono')
            ->get();
        ...
        if ($flag == true) {
            ...
            if ($reserva['tipo_reserva_estancia'] == 1) {
                ...
                foreach($huespedes as $value){
                    ...
                    $huesped->numero_vuelo=$value['numero_vuelo'];
                    $huesped->correlativo=$numeracion;
                    if($reserva['persona_id']==$value['persona_id']){
                        $huesped->estado_registro=1;
                    }
                    $huesped->save();
                    $numeracion += 1;  
                }
    ...
    public function get_reserva_estancia_cabecera_list(Request $request){
        ...
        $reserva=DB::select("SELECT re.reserva_estancia_id,re.fecha_checkout,per.persona_id, per.razon_social_nombre, ha.numero_habitacion,
		 case when paquetes_tarifa >0 then CONCAT('SERVICIO DE HOSPEDAJE',' + ','PAQUETE TURISTICO')
				when paquetes_tarifa >0 AND (reh.late_checkout <> NULL or reh.late_checkout=0) then CONCAT('SERVICIO DE HOSPEDAJE',' + ','PAQUETE TURISTICO',' + ','LATE CHECKOUT')
				when paquetes_tarifa >0 AND (reh.late_checkout <> NULL or reh.late_checkout=0) AND (early_checkin <> NULL or reh.early_checkin=0)then CONCAT('SERVICIO DE HOSPEDAJE',' + ','PAQUETE TURISTICO',' + ','LATE CHECKOUT',' + ','EARLY CHECKIN')
				when paquetes_tarifa =0 AND (reh.late_checkout <> NULL or reh.late_checkout=0) then CONCAT('SERVICIO DE HOSPEDAJE',' + ','LATE CHECKOUT')
				when paquetes_tarifa =0 AND (reh.early_checkin <> NULL or reh.early_checkin=0)then CONCAT('SERVICIO DE HOSPEDAJE',' + ','EARLY CHECKOUT')
				when paquetes_tarifa =0 AND (reh.early_checkin <> NULL or reh.early_checkin=0)AND (reh.late_checkout <> NULL or reh.late_checkout=0) then CONCAT('SERVICIO DE HOSPEDAJE',' + ','LATE CHECKOUT',' + ','EARLY CHECKOUT')
		ELSE 'SERVICIO DE HOSPEDAJE' end AS concepto,
(reh.tarifa+re.paquetes_tarifa+ifnull(reh.late_checkout,0)+ifnull(reh.early_checkin,0)) AS precio,re.estado_pago  FROM reserva_estancia re
INNER JOIN reserva_estancia_habitacion reh ON reh.reserva_estancia_id=re.reserva_estancia_id
INNER JOIN persona per ON per.persona_id=re.persona_id
INNER JOIN habitacion ha ON ha.habitacion_id = reh.habitacion_id
WHERE re.estado_pago =0;");  
        $reservaT=[];
   ...
   public function get_resumen_reserva_estancia(Request $request){
        ...
        $reserva = DB::table('reserva_estancia as re')       
            ...
            ,'per.razon_social_nombre','per.numero_documento','reh.tarifa',
        ...
        $consumo=DB::table('pedido_nota_consumo_maestro as pncm')
            ...
            ->where([['pncm.reserva_estancia_id','=',$request->get('param')],['pncm.tipo_consumo','=','C']])
            ->get();
        $consumoP= DB::table('pedido_nota_consumo_maestro as pncm')
            ->join('pedido_nota_consumo_maestro_detalle as pncmd','pncmd.pedido_nota_consumo_maestro_id','=','pncm.pedido_nota_consumo_maestro_id')
              
            ->join('producto as p','p.producto_id','=','pncmd.producto_id')
            ->join('unidad_medida as um','um.unidad_medida_id','=','pncmd.unidad_medida_id')
            ->select('pncmd.*',DB::raw('p.denominacion,um.abreviatura'))     
            ->where([['pncm.reserva_estancia_id','=',$request->get('param')],['pncm.tipo_consumo','=','P']])
            ->get();
        ...
        $huespedes = DB::table('reserva_estancia_huesped')
        ...
        ->select( 'reserva_estancia_huesped.*','persona.razon_social_nombre','profesion.profesion_id','profesion.profesion','persona.menor','persona.tipo_documento_id','persona.numero_documento','persona.nacionalidad','persona.correo_electronico','persona.fecha_nacimiento','persona.telefono')
        ->where('reserva_estancia_id',$request->get('param'))
        ->get();
            if(!empty($reserva)){
            $json = array(
                "status"=>200,
     ...
     public function set_registro_reserva_estancia($reservaid){
        try {
            ReservaEstancia::where("reserva_estancia_id", $reservaid)->update(['estado_registro'=>1]);
            $json = array(
                    "status"=>200,
                    "mensaje"=>"Registro completado, Gracias por tu preferencia",
                    "detalles"=>null
                );
        } catch (\Throwable $th) {
           $json = array(
                "status"=>400,
                "mensaje"=>"Error al registrar estancia",
                "detalles"=>null
            );
        }
    ...
    public function get_registro_estacia_pdf($reservaid){
        $consumo = DB::table('pedido_nota_consumo_maestro as pncm')
            ->join('pedido_nota_consumo_maestro_detalle as pncmd','pncm.pedido_nota_consumo_maestro_id','=','pncmd.pedido_nota_consumo_maestro_id')
            ->select('pncm.reserva_estancia_id',DB::raw('SUM(pncmd.sub_total) AS subtotal'))
            ->groupBy('pncm.reserva_estancia_id');
        ...
        ifnull(reh.late_checkout,0)+ifnull(consumo.subtotal,0)),
        pt.tarifa+
        (if((DATEDIFF(date(re.fecha_salida),date(re.fecha_llegada)))>pt.numero_noches,
        (((DATEDIFF(date(re.fecha_salida),date(re.fecha_llegada)))-pt.numero_noches)*reh.tarifa),0))+
        ifnull(reh.early_checkin,0)+
        ifnull(reh.late_checkout,0)+ifnull(consumo.subtotal,0)) AS total'),
        DB::raw('if(pt.numero_noches=0,((reh.tarifa*(DATEDIFF(date(re.fecha_salida),date(re.fecha_llegada))))+
        ifnull(reh.early_checkin,0)+
        ifnull(reh.late_checkout,0)+ifnull(consumo.subtotal,0)),
        pt.tarifa+
        (if((DATEDIFF(date(re.fecha_salida),date(re.fecha_llegada)))>pt.numero_noches,
        ...
        ->leftJoin('profesion','profesion.profesion_id','=','persona.profesion')
        ->select( 'reserva_estancia_huesped.*','persona.razon_social_nombre','profesion.profesion_id','profesion.profesion','persona.menor','persona.tipo_documento_id','persona.numero_documento','persona.nacionalidad','persona.correo_electronico','persona.fecha_nacimiento','persona.telefono')
        ->where('reserva_estancia_id',$reservaid)
        ->get();
        ...
        //return json_encode($data);
        $pdf= PDF::loadView('RegistroEstancia',compact('data'));        
        $pdf->setPaper(array(0,0,595.28,420.94),'portrait');        
        return $pdf->stream('registroestancia.pdf');    
    }
    ...
    public function get_send_reserva_smtp2go_api(Request $request){
        try {
            ...
            //return json_encode($data);
            $client = new Client([
                    'base_uri' => env('URL_SMTP2GO_URL')."/email/",
                    //'timeout'  => 2.0,
                ]);
            $response  =$client->request('POST','send',['json'=>$data]);
            $response  = json_decode($response->getBody()->getContents());
            //return $response;
            if($response->data->succeeded>0){
                 $json = array(
                    "status"=>200,
                    "mensaje"=>"Correo enviado al ".$request->input('to'),
                    "detalles"=>$request->input('to')
                );
            }else{
                $json = array(
                "status"=>400,
                "mensaje"=>"Error al enviar el correo al ".$request->input('to'),
                "detalles"=>$request->input('to')
                );
            }
        } catch (Exception $e) {
