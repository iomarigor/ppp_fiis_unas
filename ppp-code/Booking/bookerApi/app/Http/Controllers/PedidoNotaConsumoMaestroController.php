...
class PedidoNotaConsumoMaestroController extends Controller
{
    ...
    public function index(){
        ...
        ->select(DB::raw("ifnull(pedd.reserva_estancia_id,0) as reserva_estancia_id,ped.pedido_nota_consumo_maestro_id,ifnull(per.persona_id,0) as persona_id,ifnull(per.razon_social_nombre,'-') AS cliente,ha.numero_habitacion,pedd.fecha_emision,sum(ped.sub_total) AS importe,pedd.tipo_consumo,pedd.empleado_id"))
        ->where('pedd.estado','=','0')
    ...
    public function show($tipo_consumo){
        $pedido = DB::table('pedido_nota_consumo_maestro as pedd')
        ->join('pedido_nota_consumo_maestro_detalle as ped','ped.pedido_nota_consumo_maestro_id','=','pedd.pedido_nota_consumo_maestro_id')
        ->leftjoin('persona as per','per.persona_id','=','pedd.persona_id')
        ->leftjoin('habitacion as ha','ha.habitacion_id','=','pedd.habitacion_id')        
        ->select(DB::raw("ifnull(pedd.reserva_estancia_id,0) as reserva_estancia_id,ped.pedido_nota_consumo_maestro_id,ifnull(per.persona_id,0) as persona_id,ifnull(per.razon_social_nombre,'-') AS cliente,ha.numero_habitacion,pedd.fecha_emision,sum(ped.sub_total) AS importe,pedd.tipo_consumo,pedd.empleado_id"))
        ->where([['pedd.estado','=','0'],['pedd.tipo_consumo','=',$tipo_consumo]])
        ->groupBy('pedd.pedido_nota_consumo_maestro_id')
        ->get(); 
        $pedidoT=[];
        foreach($pedido as $dp){
        ...
        if(!empty($pedido)){
            ...
            if ($validator->fails()) {
                ...
                foreach($pedido_detalle as $value){
                    ...
                    ->where('producto_id',$value['producto_id'])->first();
    ...
    public function del_pedido_nota_consumo_maestro(Request $request){
        ...
        if(!$validar->count()==0){     
            ...
            $productAfec=DB::table('producto')
                    ->where('producto_id',$pedido_detalleTemp->producto_id)->first();
                    if($productAfec->contabilizar==1){
            DB::table('producto')
    ...
    public function get_habitacion_reserva_modal(Request $request){
        ...
        ->join('reserva_estancia_huesped as rehd','rehd.reserva_estancia_id','=','re.reserva_estancia_id')
        ->join('habitacion as ha','ha.habitacion_id','=','reh.habitacion_id') 
        ->join('persona as per','per.persona_id','=','rehd.persona_id')               
        ->select(DB::raw("ha.habitacion_id,reh.reserva_estancia_id,per.razon_social_nombre,ha.numero_habitacion,per.persona_id,per.numero_documento"))
        ->where([
            ['re.estado','=',1],
        ...
        ->orWhere([
            ['re.estado','=',1],
            ['re.tipo_reserva_estancia','=',1],
            ['per.numero_documento','like','%'.$request->get("parametro").'%']
        ])
        ->get(); 
    ...
    public function set_pedido_nota_consumo_maestro_form_pedido(Request $request){
        ...
        $pedido = array( 
            ...
            "habitacion_id"=>$request->input("habitacion_id"),
            "reserva_estancia_id"=>$request->input("reserva_estancia_id"),
            "tipo_consumo"=>($request->input("tipo_consumo"))?$request->input("tipo_consumo"):null,        
            "empleado_id"=>$request->input("empleado_id")
        );             
        $pedido_detalle =$request->input("detalles");
        if(!empty($pedido)){
    ...
    public function get_pedido_print($id){      
       /*  $pedidoTemp=DB::table('pedido_nota_consumo_maestro as ped')         
        ->where('ped.pedido_nota_consumo_maestro_id','=',$id)
        ->get();
        
        if($pedidoTemp[0]->persona_id){ */
            $pedido = DB::table('pedido_nota_consumo_maestro as ped')         
         ->leftJoin('persona as p','p.persona_id','=','ped.persona_id')
         ->leftJoin('habitacion as h','h.habitacion_id','=','ped.habitacion_id')
        ...
        /* }else{
            $pedido = DB::table('pedido_nota_consumo_maestro as ped')         
         ->leftJoin('reserva_estancia as rs','rs.reserva_estancia_id','=','ped.reserva_estancia_id')
         ->leftJoin('persona as p','p.persona_id','=','rs.persona_id')
         ->leftJoin('habitacion as h','h.habitacion_id','=','ped.habitacion_id')
         ->select('ped.*','p.razon_social_nombre','p.numero_documento','h.numero_habitacion','p.direccion')
         ->where('ped.pedido_nota_consumo_maestro_id','=',$id)
         ->get();
        ...
        $textypost=4;
        //$fpdf->Image(public_path('img/logofacpdf_ficha.jpg'), 20, $textypost, 40, 20);
        //$textypost+=25;
        ...
        $fpdf->Cell(0,0,("HABITACIÓN: N°".$pedido[0]->numero_habitacion),0,1,"C");
        $textypost+=4; */
        ...
        $textypost += 3; */
        $fpdf->SetXY(15, $textypost);
        $fpdf->SetFont('Courier', 'B', 11);
        //$facturacion->totalventas
        $fpdf->Cell(0, 0, "TOTAL   : S/" . str_pad(number_format($total, 2, '.', ','), (10 - strlen($total)) + strlen($total), ' ', STR_PAD_BOTH));
        $textypost += 2;
        $fpdf->Line(1, $textypost, 79, $textypost);
        
        /* $texto_qr = utf8_decode($establecimiento->nombre_comercial."-".$establecimiento->numero_documento." ").
        "\nFecha/Hora: ".$pedido[0]->fecha_emision.
        "\nPedido: ".$pedido[0]->pedido_nota_consumo_maestro_id.
        "\nTotal: ".$total;
        $textypost += 5;
        $fpdf->SetXY(23, $textypost);
        ...
        $fpdf->SetTitle(utf8_decode("PEDIDO: N° ".$pedido[0]->pedido_nota_consumo_maestro_id));
    ...
    public function set_pedido_nota_consumo_maestro_form_pedido_update(Request $request){
		...
		if(!empty($datosPedido)){         
			...
			if (!$validar->count()==0){
                ...
                $pedidodetalle = PedidoNotaConsumoMaestroDetalle::where("pedido_nota_consumo_maestro_id", $idpedido)->get();
    ...
    public function set_pedido_nota_consumo_maestro_y_detalle_delete(Request $request){
        ...
        if(!$validar->count()==0){                     
            ...
            $pedidoDetalle = PedidoNotaConsumoMaestroDetalle::where("pedido_nota_consumo_maestro_id", $id)->get();
            foreach($pedidoDetalle as $value){
                $productAfec=DB::table('producto')
                    ->where('producto_id',$value['producto_id'])->first();
                if($productAfec->contabilizar==1){
