...
class FacturacionController extends Controller
{
	...
	public function pdf_imprime_comprobante($tipeid,$id){
        ...
        $facturacion = DB::table('facturacion as f')
        ->select('f.*',DB::raw('p.razon_social_nombre,p.numero_documento,p.direccion,tm.codigo,tp.tipo_pago,tfp.tipo_forma_pago,c.codigo_sunat,c.comprobante_id,sc.serie_comprobante, DATEDIFF(date(re.fecha_salida),date(re.fecha_llegada))+1 as noches,re.fecha_llegada, DATE_ADD(re.fecha_salida, interval 1 day) as fecha_salida, re.numero_adultos, re.numero_ninos, u.name'))
        ->join('persona as p','p.persona_id','=','f.persona_id')
        ->join('tipo_moneda as tm','tm.tipo_moneda_id', '=','f.tipo_moneda_id')
        ->join('tipo_operacion as top','top.tipo_operacion_id','=', 'f.tipo_operacion_id')
        ->join('tipo_pago as tp','tp.tipo_pago_id','=', 'f.tipo_pago_id')
        ...
        ->leftJoin('reserva_estancia as re','re.reserva_estancia_id','=', 'f.reserva_estancia_Id')
        ->where('f.facturacion_id','=',$id)
        ->first();
        ...
        $textypost+=4;
        $fpdf->setXY(5,$textypost);
        $fpdf->Cell(0,0,"DE:".$establecimiento->razon_social_nombres,0,1,"C");
        $textypost+=4;
        $fpdf->setXY(5,$textypost);
        $fpdf->Cell(0,0,"RUC:".$establecimiento->numero_documento,0,1,"C");
        $textypost+=4;
        $fpdf->setXY(5,$textypost);
        $fpdf->Cell(0,0,utf8_decode("DIRECCIÓN FISCAL:"),0,1,"C");
        $textypost+=4;
        $fpdf->setXY(5,$textypost);
        ...
        if($facturacion->reserva_estancia_Id!=null&&$facturacion->reserva_estancia_Id!=0){
            $textypost+=5;
            $fpdf->setXY(2,$textypost);
            $fpdf->SetFont('Courier', 'B', 8);
            $fpdf->Cell(0, 0, utf8_decode("F.LLEGADA: ") . date('d/m/Y', strtotime(explode(' ',$facturacion->fecha_llegada)[0])) . str_pad("F. SALIDA: " . date('d/m/Y', strtotime(explode(' ',$facturacion->fecha_salida)[0])), 17, ' ', STR_PAD_LEFT));
            $textypost+=4;
            $fpdf->setXY(2,$textypost);
            $fpdf->SetFont('Courier', 'B', 8);
            $fpdf->Cell(0, 0, utf8_decode("# Adultos: ") . $facturacion->numero_adultos . str_pad(utf8_decode("# Niños: ") .  $facturacion->numero_ninos, 15, ' ', STR_PAD_LEFT).str_pad(utf8_decode("# Noches: ") . $facturacion->noches,15, ' ', STR_PAD_LEFT));
        ...
        foreach($facturacionDetalle as $rM){
            ...
            $fpdf->SetXY(2, $textypost);
            /* if($rM->unidad_medida_id==69){
                $fpdf->MultiCell(0, 3, '*' . utf8_decode("SERVICIO DE ALOJAMIENTO"));
            }else{ */
        ...
        $textypost += 5;
        $fpdf->SetXY(23, $textypost);
        $fpdf->MultiCell(0, 3, utf8_decode("CAJERO: ".$facturacion->name." \nTIPO PAGO: ".$facturacion->tipo_pago."\nFORMA DE PAGO: ".$facturacion->tipo_forma_pago."\n\nRepresentación impresa del comprobante electrónico, esta puede ser consultada en ...."));
        ...
        if($facturacion->observacion!=null&&strlen($facturacion->observacion)!=0){
            $fpdf->SetFont('Courier', 'B', 7);
            $textypost += 2;
            $fpdf->SetXY(9, $textypost);
            $fpdf->Cell(0, 0, utf8_decode("OBSERVACIÓN:"), 0, 1, "C");
            $textypost +=3;
            $fpdf->SetXY(9, $textypost);
            $fpdf->Cell(0, 0, utf8_decode(strtoupper($facturacion->observacion)), 0, 1, "C");
            $y5 = $fpdf->GetY();
            $textypost = $y5 + 2;
            $fpdf->Line(1, $textypost, 79, $textypost);
            $fpdf->SetFont('Courier', 'B', 8);
