...
export const LOADER_SEND_MAIL = "[GRID] LOADER SEND MAIL";
export const LOADER_GUEST_REGISTER = "[GRID] LOADER GUEST REGISTER";
export const LOADER_COMFIR_REGISTER = "[GRID] LOADER COMFIR REGISTER";
...
export function getAllBookings(id = null) {
  const request = axios.get(
    `${process.env.REACT_APP_API_URL}/api/reserva/${id ? id : ""}`
  );
  ...
  return (dispatch) =>
    request.then((response) => {
      ...
      if (response.data.status !== 200) {
        ...
        dispatch(Actions.getAllConsumeByRoom(form.reserva_estancia_id, "C"));
      }
...
export function saveGuestByBookingRegister(form) {
  toast.info("Registrando estancia...", {
    toastId: "registerGuest",
    autoClose: 20000,
  });
  const request = axios.post(
    `${process.env.REACT_APP_API_URL}/api/restanciahuespedr`,
    form
  );
  ...
  return (dispatch) =>
    request.then((response) => {
      ...
      dispatch(Actions.getResumeByBooking(form.reserva_estancia_id));
      dispatch({
        type: SAVE_GUEST_BY_BOOKING,
      });
      return dispatch(loaderGuestRegister(false));
    });
}
export function saveRegisterEstancia(form) {
  toast.info("Registrando reserva...", {
    toastId: "registerEstancia",
    autoClose: 20000,
  });
  const request = axios.get(
    `${process.env.REACT_APP_API_URL}/api/restancia/${form}`
  );
  ...
  return (dispatch) =>
    request.then((response) => {
      ...
      dispatch(Actions.getResumeByBooking(form));
      return dispatch(loaderComfirRegister(false));
      /* dispatch({
        type: SAVE_GUEST_BY_BOOKING,
      }); */
    });
}
export function loaderGuestRegister(status) {
  return (dispatch) =>
    dispatch({
      type: LOADER_GUEST_REGISTER,
      payload: status,
    });
}
export function loaderComfirRegister(status) {
  return (dispatch) =>
    dispatch({
      type: LOADER_COMFIR_REGISTER,
      payload: status,
    });
}
...
export function loaderSendMail(status) {
  return (dispatch) =>
    dispatch({
