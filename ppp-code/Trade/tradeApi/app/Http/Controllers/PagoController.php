...
class PagoController extends Controller
{
    ...
    public function registrar_pagos(Request $request){
        ...
        if(!empty($datos)){
            ...
            /*$saldos = DB::table('reserva_estancia as re')
                ...
                ->leftJoin('pedido_nota_consumo_maestro as pncm','pncm.reserva_estancia_id','=','re.reserva_estancia_id')
                ->leftJoin('pedido_nota_consumo_maestro_detalle as pncmd','pncmd.pedido_nota_consumo_maestro_id','=','pncm.pedido_nota_consumo_maestro_id')
                ->leftJoin('pago as pa','pa.reserva_estancia_id','=','re.reserva_estancia_id')
                ->where('re.reserva_estancia_id','=',$datos['reserva_estancia_id'])
                ->select(DB::raw('(reh.tarifa+pt.tarifa+ifnull(sum(pncmd.sub_total),0))-(ifnull(SUM(pa.monto_deposito),0)) AS saldo'))
                ->groupBy('reh.tarifa','pt.tarifa')
                ->get();
           $saldon=0;
    ...
    public function mostrar_pagos_reserva_estancia($id){
        $reserva_id=$id;
        $pagos = DB::table('pago')
        ->where('pago.reserva_estancia_id',$reserva_id)
        ->get();
